...
- 为迁移旧版 `sparse`，用户手动重写该旧类型时会按当前 LivingMemory 数量重新分档，保留原当天聊天来源，并为新的 low_activity 重新选择历史记忆；不会自动改写旧日记。
- 管理页时间线会显示 `entry_type`；low_activity 额外显示当天轮数、聊天素材数和随机回忆来源数，不直接暴露内容。

新增配置：`recent_context_days`（默认 3）、`historical_memory_min_count` / `historical_memory_max_count`（默认 3/5）、`reflection_cooldown_days`（默认 30）。权重实现保持内置，避免配置膨胀。
